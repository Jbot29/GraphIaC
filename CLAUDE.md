...
- `src/GraphIaC/web/registry.js` — GENERATED. All AWS type knowledge for the JS parser, introspected from the Pydantic models. Regenerate after changing any model or `model_map.py`: `python -m GraphIaC.dsl_registry`. The name-field and edge-endpoint tables live in `dsl_registry.py`; new node/edge classes must be added there too to be usable from the DSL. Subclass nodes (DeployRole isa IAMRole) inherit their parent's edge endpoints and guard targets via the registry `isa` chain — both parsers walk it.
