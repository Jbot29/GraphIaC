...
1. **`registry.json`** — generated by Python from the Pydantic models and
   `model_map.py`: every node type's fields, defaults, and name field; every
   edge type's endpoint types, endpoint field names, and canonical direction.
   The JS parser consumes this file; it never hand-codes AWS knowledge.
